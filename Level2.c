...
void Load2(void)
{
	...
	Enemy[1].pMesh = AEGfxMeshEnd();
	Enemy[1].width = 30;
	Enemy[1].height = 25;
	Enemy[1].outo = 1;
	AE_ASSERT_MESG(Enemy[1].pMesh, "Failed to create mesh 2!!");
	...
	// Texture 1: From file
	pTex1 = AEGfxTextureLoad("walk1.png");
...
void Ini2(void)
{
	...
	// 墙体3的初始位置
	Wall[1].posCurr.x = 550.0f;
	Wall[1].posCurr.y = -40.0f;

	// 墙体4的初始位置
	Wall[2].posCurr.x = 300.0f;
	Wall[2].posCurr.y = 100.0f;

	// 墙体5的初始位置
	Wall[3].posCurr.x = 780.0f;
	Wall[3].posCurr.y = 100.0f;

	// 墙体6的初始位置
	Wall[4].posCurr.x = 1500.0f;
	Wall[4].posCurr.y = 100.0f;

	// 墙体7的初始位置
	Wall[5].posCurr.x = -200.0f;
	Wall[5].posCurr.y = 100.0f;

	// 墙体8的初始位置
	Wall[6].posCurr.x = 1600.0f;
	Wall[6].posCurr.y = -200.0f;


	Enemy[0].posCurr.x = -200.0f;
	Enemy[0].posCurr.y = -170.0f;

	Enemy[1].posCurr.x = 100.0f;
	Enemy[1].posCurr.y = -170.0f;
...
void Update2(void)
{
	...
	for (i = 0; i <= 1; i++) {
		if (StaticRectToStaticRect(&Hero.posCurr, Hero.width, Hero.height, &Enemy[i].posCurr, Enemy[i].width, Enemy[i].height)) FLAG[0] = 1;
	}
	...
	if (Wall[1].outo == 3) {
		if (StaticRectToStaticRect(&Hero.posCurr, Hero.width, Hero.height, &Wall[1].posCurr, Wall[1].width, Wall[1].height) == 3)
			Hero.posCurr.y += frameTime * 100.0f;
		Wall[1].posCurr.y += frameTime * 100.0f;
		if (Wall[1].posCurr.y >= 200) Wall[1].outo = 4;
	}
	if (Wall[1].outo == 4) {
		if (StaticRectToStaticRect(&Hero.posCurr, Hero.width, Hero.height, &Wall[1].posCurr, Wall[1].width, Wall[1].height) == 3)
			Hero.posCurr.y -= frameTime * 100.0f;
		Wall[1].posCurr.y -= frameTime * 100.0f;
		if (Wall[1].posCurr.y <= -200) Wall[1].outo = 3;
	}

	if (Wall[3].outo == 1) {
		if (StaticRectToStaticRect(&Hero.posCurr, Hero.width, Hero.height, &Wall[3].posCurr, Wall[3].width, Wall[3].height) == 3)
			Hero.posCurr.x += frameTime * 80.0f;
		Wall[3].posCurr.x += frameTime * 80.0f;
		if (Wall[3].posCurr.x >= 1200) Wall[3].outo = 2;
	}
	if (Wall[3].outo == 2) {
		if (StaticRectToStaticRect(&Hero.posCurr, Hero.width, Hero.height, &Wall[3].posCurr, Wall[3].width, Wall[3].height) == 3)
			Hero.posCurr.x -= frameTime * 80.0f;
		Wall[3].posCurr.x -= frameTime * 80.0f;
		if (Wall[3].posCurr.x <= 780) Wall[3].outo = 1;
	}

	if (Enemy[0].outo == 1) {
		if (StaticRectToStaticRect(&Hero.posCurr, Hero.width, Hero.height, &Enemy[0].posCurr, Enemy[0].width, Enemy[0].height) == 3)
			Hero.posCurr.x += frameTime * 150.0f;
		Enemy[0].posCurr.x += frameTime * 150.0f;
		if (Enemy[0].posCurr.x >= 0) Enemy[0].outo = 2;
	}
	if (Enemy[0].outo == 2) {
		if (StaticRectToStaticRect(&Hero.posCurr, Hero.width, Hero.height, &Enemy[0].posCurr, Enemy[0].width, Enemy[0].height) == 3)
			Hero.posCurr.x -= frameTime * 150.0f;
		Enemy[0].posCurr.x -= frameTime * 150.0f;
		if (Enemy[0].posCurr.x <= -200) Enemy[0].outo = 1;
	}

	if (Enemy[1].outo == 1) {
		if (StaticRectToStaticRect(&Hero.posCurr, Hero.width, Hero.height, &Enemy[1].posCurr, Enemy[1].width, Enemy[1].height) == 3)
			Hero.posCurr.x += frameTime * 150.0f;
		Enemy[1].posCurr.x += frameTime * 150.0f;
		if (Enemy[1].posCurr.x >= 300) Enemy[1].outo = 2;
	}
	if (Enemy[1].outo == 2) {
		if (StaticRectToStaticRect(&Hero.posCurr, Hero.width, Hero.height, &Enemy[1].posCurr, Enemy[1].width, Enemy[1].height) == 3)
			Hero.posCurr.x -= frameTime * 150.0f;
		Enemy[1].posCurr.x -= frameTime * 150.0f;
		if (Enemy[1].posCurr.x <= 100) Enemy[1].outo = 1;
	}
...
void Draw2(void)
{
	...
	AEGfxSetRenderMode(AE_GFX_RM_TEXTURE);   // 必须最先设置绘制模式为RGB
	AEGfxSetPosition(Enemy[1].posCurr.x, Enemy[1].posCurr.y);
	// Set texture for object 2
	AEGfxTextureSet(pEnemy, 0, 0);
	AEGfxSetTransparency(1.0f);
	AEGfxSetBlendColor(0.0f, 0.0f, 0.0, 0.0f);
	// Drawing the mesh (list of triangles)
	AEGfxMeshDraw(Enemy[1].pMesh, AE_GFX_MDM_TRIANGLES);
